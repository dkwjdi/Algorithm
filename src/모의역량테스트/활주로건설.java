   ...
   private static void solve(int map[][]) {
      ...
      for (int i = 0; i < N; i++) {
        ...
        outer: for (int j = 0; j < N ; j++) {
            ...
            if (map[i][j] == map[i][j + 1] ) continue; // 같으면
            if (Math.abs(map[i][j] - map[i][j + 1]) > 1)
               break;
            else { // 지금이랑 그 다음이랑 1개 차이날 때
               if (map[i][j] - map[i][j + 1] > 0) {// 낮아질때
                  if (widthcheck(i, j,map) == false) break;
                  else { // 활주로 놓여져 잇는지 검사 
                	  for(int k=j; k<j+X; k++) {
                		  if(visited[k+1]==false) visited[k+1]=true;
                		  else break outer;
                	  }
                  }
                  	
               } else if(map[i][j]-map[i][j+1]<0){ // 높아지면
                  if (widthcheck2(i, j,map) == false) break;
                  else {
                	  for(int k=j; k>j-X; k--) {
